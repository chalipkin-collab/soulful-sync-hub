...
export default function ThemeSettings() {
  ...
  return (
    <div className="glass-card p-4 flex flex-col gap-4">
      <h3 className="text-sm font-semibold text-right">הגדרות תצוגה</h3>
        ...
        {COLORS.map(c => (
          <button
            ...
            className={`w-8 h-8 rounded-full ${c.class} transition-all ${
              color === c.id ? "ring-2 ring-foreground ring-offset-2 ring-offset-background scale-110" : "opacity-70 hover:opacity-100"
            }`}
            title={c.label}
          />
        ))}
